...
<html>
    ...
    <h1>Catogory Management Table</h1>
        ...
        <?php
            ...
            $connect=mysqli_connect("localhost","root","",'foodparadies');
            if(!$connect){
                die("<p>Connection Fail: ".mysqli_error($connect)."</p>");
            }
            else{
                    
                $get_value='SELECT * FROM category;';
                $result = mysqli_query($connect,$get_value);
        ...
        if(isset($_POST['submit'])){
            if(isset($_POST['name'])){
                ...
                header("refresh:0");
                mysqli_query($connect,$sql_valuee);
            ...
            if (isset($_POST["delete"])) {
                if(isset($_POST['del_id'])){
                    ...
                    mysqli_query($connect,"DELETE FROM Category WHERE CategoryName='$row_name';");
                    header("refresh:0");
                    echo '<p>Item iS deleted<p>';
                }
                else{
                    echo '<p>Not delete '.mysqli_error($connect).'<p>';
                }
            }
